...
class HashObject; // for Google's benchmark, not in spp namespace!
...
// ------------------------------------------------------------------------
	...
	template<int S, int H>
	struct is_relocatable<HashObject<S, H> > : true_type {
	};
	...
	template<class T>
	struct is_relocatable<volatile T> : is_relocatable<T> {
	};
	template<class T>
	struct is_relocatable<const volatile T> : is_relocatable<T> {
	};
