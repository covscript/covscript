...
// Hash
	...
	{
		static cs::namespace_t null_ext;
		return null_ext;
	}
	...
	namespace operators {
		...
		enum class type : unsigned
		{
			...
			destroy = 3,
			get = 4,         // .val<T>()
			type_id = 5,     // typeid(val)
			type_name = 6,   // type(val)
			to_integer = 7,  // to_integer(val)
			to_string = 8,   // to_string(val)
			hash = 9,        // hash(val)
			detach = 10,     // clone(val)
			ext_ns = 11,     // helper for access
			add = 12,        // lhs + rhs
			sub = 13,        // lhs - rhs
			mul = 14,        // lhs * rhs
			div = 15,        // lhs / rhs
...
#define CS_VAR_SVO_ALIGN (2 * sizeof(void *))
	...
	public:
		...
		cs::namespace_t &get_ext() const
		{
			...
			cs::namespace_t &ext = this->mDat->data.get_ext();
			if (!ext)
				throw cs::runtime_error(std::string("Target type \"") + cxx_demangle(this->mDat->data.get_type_name()) + "\" doesn't have extension field.");
			return ext;
		}

		cs::namespace_t get_ext_opt() const noexcept
		{
			if (this->mDat == nullptr)
				return {};
