...
static int
read_central_dir_values(CZipFile* file, const unsigned char* buf, int len)
{
    if (len < EOCD_LEN) {
    ...
    file->disknum = static_cast<unsigned short>(read_le_short(&buf[0x04]));
    file->diskWithCentralDir = static_cast<unsigned short>(read_le_short(&buf[0x06]));
    file->entryCount = static_cast<unsigned short>(read_le_short(&buf[0x08]));
    file->totalEntryCount = static_cast<unsigned short>(read_le_short(&buf[0x0a]));
    file->centralDirSize = read_le_int(&buf[0x0c]);
    file->centralDirOffest = read_le_int(&buf[0x10]);
    file->commentLen = static_cast<unsigned short>(read_le_short(&buf[0x14]));
...
static int
read_central_directory_entry(CZipFile* file, CZipEntry* entry,
                const unsigned char** buf, const ssize_t* len)
{
    ...
    entry->compressionMethod = static_cast<unsigned short>(read_le_short(&p[0x0a]));
    ...
    extraFieldLength = static_cast<unsigned short>(read_le_short(&p[0x1e]));
    fileCommentLength = static_cast<unsigned short>(read_le_short(&p[0x20]));
    ...
    *buf = p;
    ...
    extraFieldLength = static_cast<unsigned short>(read_le_short(&p[0x1c]));

    dataOffset = static_cast<unsigned int>(localHeaderRelOffset + LFH_SIZE
 ...
 */
int
read_central_dir(CZipFile *file)
{
    int err;

    const unsigned char* buf = file->buf;
    ZD_TYPE bufsize = file->bufsize;
    const unsigned char *eocd = nullptr;
    const unsigned char* p;
    ...
    // extract eocd values
    err = read_central_dir_values(file, eocd, static_cast<int>((buf + bufsize) - eocd));
    if (err != 0) {
        goto bail;
    }
    ...
    for (i=0; i < file->totalEntryCount; i++) {
        auto * entry = static_cast<CZipEntry *>(malloc(sizeof(CZipEntry)));
        memset(entry, 0, sizeof(CZipEntry));
