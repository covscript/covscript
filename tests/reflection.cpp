...
CNI_ROOT_NAMESPACE {
    ...
    CNI_V(modify_mem_fn, [](cs::object_method& om, const cs::var& target){
        if (target.type() == typeid(cs::object_method))
            modify_function(om.callable, target.val<cs::object_method>().callable.const_val<cs::callable>().get_raw_data(), cs::callable::types::member_fn);
        else if (target.type() == typeid(cs::callable))
            modify_function(om.callable, target.const_val<cs::callable>().get_raw_data(), cs::callable::types::member_fn);
        else
        ...
        type.constructor = [type, map]()->cs::var {
            ...
            for (auto& it:map)
            {
                ...
                if (it.second.type() == typeid(cs::object_method))
                    modify_function(om, it.second.val<cs::object_method>().callable.const_val<cs::callable>().get_raw_data(), cs::callable::types::member_fn);
                else if (it.second.type() == typeid(cs::callable))
                    modify_function(om, it.second.const_val<cs::callable>().get_raw_data(), cs::callable::types::member_fn);
                else
                    throw cs::lang_error("Not a callable object!");
            }
            return cs_struct;
