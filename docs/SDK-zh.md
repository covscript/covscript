...
逃逸对象需要执行脚本代码时，应保持 context 存活。自包含的值数据（结构体成员字段）可独立继续使用。
...
`cs::var` 是一个指针大小的句柄（64 位平台上为 8 字节）；拷贝它只会让引用计数 +1，值在最后一个引用释放时被回收。要把值与其原始存储分离，请使用 `cs::copy(var)`（深拷贝）。逃逸出 context 的自包含值数据仍然安全；脚本 callable 持有非拥有函数指针，context 销毁后调用是未定义行为；结构体成员数据（域）仍有效，但其类型身份和脚本方法同样受未定义行为约束。
