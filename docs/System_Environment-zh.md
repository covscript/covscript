...
**环境变量**：`COVSCRIPT_DEBUG`

控制防御性运行时守卫的行为（例如销毁未完成的 fiber、程序入口处函数值栈非空，或结构体 finalize 方法在清理阶段抛出异常）。取值大小写不敏感；未设置或非法值默认按 `warning` 处理。

| 取值 | 行为 |
| :-- | :-- |
| `none` | 静默忽略：程序继续运行，接受部分泄漏 |
| `warning` | 向 stderr 输出警告并继续（默认） |
| `strict` | 输出警告后立即中止（fail-fast） |

守卫仅用于诊断，在正常路径下不改变运行时语义。无论模式如何都必须执行的恢复动作（例如程序入口处清理陈旧的函数值栈）将无条件执行；模式仅控制诊断输出。
